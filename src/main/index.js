...
<style>
  ...
  /* The update line keeps its height from the first paint, so the answer arriving
     from the network never shifts what is already on screen. */
  .upd{margin:0;font-size:12px;color:#9494a0;min-height:16px}
</style>
<div class="wrap">
  <h1>PensaGrex</h1>
  <div class="ver">Version ${pkg.version}</div>
  <div class="upd" id="upd">Checking for a newer version…</div>
  ...
  // The version check runs here and only here: opening About is the act that asks
  // for it, so the app never reaches the network on its own. The window is already
  // up, so a slow or dead connection costs nothing but the line staying quiet.
  // Main does the fetching and the window is told the answer, which keeps the
  // request out of a page that is otherwise entirely local.
  const win = _aboutWin
  checkForUpdate({ currentVersion: pkg.version }).then((r) => {
    if (!win || win.isDestroyed()) return
    // Rendered by the page from data, never by interpolating text into markup:
    // the version and the URL come from a third party, and textContent cannot be
    // made to mean anything but text.
    win.webContents.executeJavaScript(`(function(r){
      var el = document.getElementById('upd'); if (!el) return;
      el.textContent = '';
      if (r.state === 'update') {
        var a = document.createElement('a');
        a.href = r.url; a.target = '_blank'; a.rel = 'noopener';
        a.textContent = 'PensaGrex ' + r.version + ' is available.';
        el.appendChild(a);
      } else if (r.state === 'current') {
        el.textContent = 'PensaGrex is up-to-date.';
      } else {
        el.textContent = 'Could not check for a newer version.';
      }
