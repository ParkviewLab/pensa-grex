...
export function createViewport({ viewportEl, worldEl, pctEl, getBounds, minScale = 0.2, maxScale = 3 }) {
  ...
  function onPointerDown(e) {
    ...
    // A press that lands on a card or a junction handle is a drag-and-drop grab
    // (interaction/drag.js), not a pan — pan is the empty-canvas gesture.
    if (e.target && e.target.closest && e.target.closest('.card, .jx-hit')) return
